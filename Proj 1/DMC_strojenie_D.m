...
D_pocz=200; %horyzont dynamiki (D)
...
wektor_wskaznikow=zeros(D_pocz-10/5,5);
...
for D=D_pocz:-5:10
    ...
    Y=Y(1:kk-N);
    U=U(1:kk-N);
    fname = sprintf('strojenie_D=%d_E=%d.mat', D, wskaznik_jakosci);    
    save(fname, 'D', 'N', 'Nu', 'lambda', 'U', 'Y', 'yzad', 'wskaznik_jakosci', 'kk');
end
%wyniki symulacji
% save('wskazniki_strojenie_D.mat', 'wektor_wskaznikow');
