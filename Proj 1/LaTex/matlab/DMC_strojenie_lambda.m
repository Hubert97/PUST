...
lambda_pocz=10; %parametr lambda np. 1
D=150; %horyzont dynamiki (D)
N=35;%horyzont predykcji (N)
Nu=5; %horyzont sterowania (Nu)(ilosc przyszlych przyrostow wartosci sterowania)
i_stroj=1;  %iterator wykorzystany do zapisywania wartosci wskaznika regulacji
wektor_wskaznikow=zeros(lambda_pocz-2,5);
%D=200; N=50; Nu=10; % robocze parametry
for lambda=lambda_pocz:-1:1
    U=zeros(1, kk);
    u=zeros(kk);
    U(:)=Upp;
    e=zeros(1, kk);
    ...
    fname = sprintf('strojenie_lambda=%d_E=%d.mat', lambda, wskaznik_jakosci);    
    save(fname, 'D', 'N', 'Nu', 'lambda', 'U', 'Y', 'yzad', 'wskaznik_jakosci', 'kk');
end
%wyniki symulacji
 save('wskazniki_strojenie_lambda.mat', 'wektor_wskaznikow');
