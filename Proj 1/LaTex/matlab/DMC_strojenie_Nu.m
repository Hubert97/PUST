...
Nu_pocz=30; %horyzont sterowania (Nu)(ilosc przyszlych przyrostow wartosci sterowania)
i_stroj=1;  %iterator wykorzystany do zapisywania wartosci wskaznika regulacji
wektor_wskaznikow=zeros(Nu_pocz-2,5);
...
for Nu=Nu_pocz:-1:2
    ...
    Y=Y(1:kk-N);
    U=U(1:kk-N);
    fname = sprintf('strojenie_Nu=%d_E=%d.mat', Nu, wskaznik_jakosci);    
    save(fname, 'D', 'N', 'Nu', 'lambda', 'U', 'Y', 'yzad', 'wskaznik_jakosci', 'kk');
end
%wyniki symulacji
 save('wskazniki_strojenie_Nu.mat', 'wektor_wskaznikow');
